...
function renderOnlineUsers(presences) {
  const content = Presence.list(presences, (
    id, { metas }
  ) => `${id} (count: ${metas.length})`).join('\n');
...
const presences = (o => proc => Object.assign(o, proc(o)))({});

channel.on('presence_state', payload => {
  renderOnlineUsers(
    presences(state => Presence.syncState(state, payload))
  );
});

channel.on('presence_diff', payload => {
  renderOnlineUsers(
    presences(state => Presence.syncDiff(state, payload))
  );
});
...
// channel.on('user_joined', console.log);
